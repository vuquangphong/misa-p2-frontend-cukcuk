<template>
  <div class="add-form-container" v-show="isOpenExtraForm">
                      ...
                      <div class="input">
                        <div class="input-container">
                          ...
                          >
                            <input
                              type="text"
                              v-model="formInfo.FoodPlaceName"
    ...
    <DuplicatedMessage
      ...
      :currentCode="formInfo.FoodPlaceName"
      :model="'Nơi chế biến'"
...
export default {
  ...
  data() {
    return {
      ...
      formTitle: resourceCukcuk.VI.formLabels.titleFormAddPlace,
  ...
  methods: {
    ...
    focusoutEvent(field, required) {
      ...
      if (required) {
        if (!this.formInfo[`FoodPlace${field}`]) {
     ...
     */
    async addNewFoodPlace() {
      const cur = this;

      cur.alertInterrupt = false;

      let formPost = {
        FoodPlaceName: cur.formInfo.FoodPlaceName,
        Description: cur.formInfo.Description,
      };

      // Validate Compulsory fields
      if (!formPost.FoodPlaceName) {
      ...
      // Check if FoodPlaceName is duplicated
      if (cur.isDuplicatedName(formPost.FoodPlaceName)) {
        cur.alertInterrupt = true;
      ...
      try {
        ...
        const res = await createModel("v1", "FoodPlaces", formPost);

        if (
          res.data.customStatusCode === enumCukcuk.customizeStatusCode.created
        ) {
          cur.$store.dispatch("getFoodPlace");
          cur.alertInterrupt = false;
        }
    ...
    /**
     * Check if the FoodPlaceName is duplicated.
     * Author: VQPhong (19/07/2022)
     */
    isDuplicatedName(currentName) {
      let tempName = currentName.toLowerCase();
      for (const place of this.foodPlace) {
        if (tempName === place.FoodPlaceName.toLowerCase()) {
          return true;
        }
      }
      return false;
